...
let createdID="0c258334-2d77-4a13-9bfd-42bc576c2845";
...
const displayCreatedChar = async (createdID) => {
  ...
  let fetchResponse = await fetch(
      `https://character-database.becode.xyz/characters/${createdID}`);

  if(!fetchResponse.ok) {
    throw new Error ("This ID doesn't exist");
  }else{
  let character = await fetchResponse.json();
...
window.onload = displayCreatedChar(createdID);
